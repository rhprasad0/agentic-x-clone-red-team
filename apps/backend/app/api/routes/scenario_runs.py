...
from app.api.dto import event_dto, finding_dto, scenario_run_dto
...
def list_scenario_runs(db: Annotated[Session, Depends(get_db_session)]) -> dict[str, list[dict]]:
    ...
    return {"items": [scenario_run_dto(run) for run in runs]}
...
def list_scenario_run_events(
    ...
    return {"items": [event_dto(event) for event in events]}
