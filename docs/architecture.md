...
- Later production-like layer: a bounded AWS/EKS deployment with public ALB, private workers, public GHCR images, managed secret storage, IRSA/OIDC, CloudWatch, and cost guardrails. The Terraform baseline lives under `infra/terraform/eks-demo`; it is not deployment evidence until planned/applied and verified with public-safe receipts.
